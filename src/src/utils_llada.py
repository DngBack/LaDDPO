...
def compute_diffusion_score(
    ...
    for _ in range(num_samples):
        ...
        noisy_input, p_mask, mask_indices = forward_process(
            input_ids, prompt_index, mask_id
        )
...
def load_llada_model_and_tokenizer(
    ...
    if load_in_8bit or load_in_4bit:
        try:
            ...
            logger.info(
                f"Using quantization: 8-bit={load_in_8bit}, 4-bit={load_in_4bit}"
            )
        except ImportError:
    ...
    model = model.to(torch_dtype)  # Cast fp32 lora params to bf16
...
def prepare_hh_rlhf_batch(
    batch, tokenizer, max_length: int = 2048, device: str = "cuda"
):
    """
