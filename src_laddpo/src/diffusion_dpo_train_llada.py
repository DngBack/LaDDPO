...
def parse_args():
    parser = argparse.ArgumentParser(description="Train a LLaDA model with DPO using HH-RLHF")
    parser.add_argument("--config_file", type=str, default="config/config_llada_8b_hh.yaml", help="Path to config YAML file")
    # Add arguments that might not be in the config file or need overriding
    parser.add_argument("--model_name_or_path", type=str, default="GSAI-ML/LLaDA-8B-Instruct", help="HF identifier for the LLaDA model")
    parser.add_argument("--dataset_name", type=str, default="Anthropic/hh-rlhf", help="HF identifier for the preference dataset")
    parser.add_argument("--dataset_split", type=str, default="train", help="Dataset split to use (e.g., train)")
    parser.add_argument("--output_dir", type=str, help="Directory to save checkpoints and logs")
    parser.add_argument("--learning_rate", type=float, default=5e-7, help="Optimizer learning rate")
    ...
    parser.add_argument("--diffusion_samples", type=int, default=16, help="Number of MC samples for diffusion score calculation")
    parser.add_argument("--use_mixed_precision", type=str, default="bf16", help="Whether to use mixed precision (","fp16",", "bf16",", "no",")")
    parser.add_argument("--gradient_checkpointing", action="store_true", default=True, help="Enable gradient checkpointing")
    parser.add_argument("--load_in_8bit", action="store_true", default=False, help="Load model in 8-bit precision")
    parser.add_argument("--load_in_4bit", action="store_true", default=False, help="Load model in 4-bit precision")
    ...
    parser.add_argument("--lr_scheduler_type", type=str, default="cosine", help="The scheduler type to use.")
    ...
    if args.model_name_or_path is None:
        raise ValueError("Model name or path must be provided (--model_name_or_path or in config).")
...
def prepare_preference_dataloader(args, tokenizer, accelerator):
    ...
    logger.info(f"Loading preference dataset {args.dataset_name} split {args.dataset_split}")
    try:
    ...
    def collate_fn(batch_list):
        ...
        batch_dict = {key: [item[key] for item in batch_list] for key in batch_list[0].keys()}
        # Prepare the batch using the utility function
        return prepare_hh_rlhf_batch(batch_dict, tokenizer, args.max_length, device="cpu") # Prepare on CPU
...
def main():
    ...
    if args.gradient_checkpointing:
        if hasattr(model, "gradient_checkpointing_enable"):
            ...
            logger.warning("Model does not support gradient_checkpointing_enable method or _gradient_checkpointing flag.")
